...
このプロジェクトでは、`.clinerules` ファイルで AI エージェント（Claude）の動作ルールを定義しています。

### Git 操作に関するルール

- **自動コミットの禁止**: AI は `git commit` を勝手に実行しません
- **必ず確認を取る**: コミット前に必ずユーザーに確認を求めます
- **承認方法**: "y", "yes", "はい", "実行" のいずれかで明示的に承認

### 使用方法

AI にコミットを依頼すると、以下のような流れになります：

```
ユーザー: 「変更をコミットして」
AI: 「以下のコミットメッセージで実行します：
     [コミットメッセージを提示]
     このコミットメッセージで実行してよろしいですか？ (y/N)」
ユーザー: 「y」
AI: コミット実行
```
